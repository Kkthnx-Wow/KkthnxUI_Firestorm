...
function Module:CreatePulseCD()
	if not C["ActionBar"].PulseCD then return end
	...
	K:RegisterEvent("SPELL_UPDATE_COOLDOWN", function()
		for i, cd in pairs(cooldowns) do
			if cd.type == "spell" then
				...
				if cdInfo and cdInfo.startTime then cd.start = cdInfo.startTime cd.duration = cdInfo.duration end
			elseif cd.type == "item" then
				local start, duration = C_Item_GetItemCooldown(i)
				if start then cd.start = start cd.duration = duration end
	...
	K:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED", function()
	...
	K:RegisterEvent("PLAYER_ENTERING_WORLD", function()
		...
		if instanceType == "arena" then
			...
			for i, v in pairs(cooldowns) do ReleaseTable(v); cooldowns[i] = nil end
			for i, v in pairs(watching) do ReleaseTable(v); watching[i] = nil end
		end
	end)
